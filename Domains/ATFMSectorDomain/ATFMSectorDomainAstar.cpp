#include "ATFMSectorDomainAstar.h"
...
//					connection_time.push_back(distance(agent_locs[i],agent_locs[j])); // use Euclidean instead of Manhattan
	...
	// JEN: Astar implementation, edge weights are (1+agent_action)*connection_time only, no variances
	weights = matrix3d(2) ;
	...
	for (unsigned i=0; i<w_mean.size(); i++){
		...
		w_var[i] = DotMultiply(connection_time,0.0);//**************************************************
	}
	weights[0] = w_mean ;
	weights[1] = w_var ;
...
//		return matrix1d(n_agents, G); // global reward
		...
		return globalPerformance ; // return D ; for difference reward training
	}
}
